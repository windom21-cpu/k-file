...
最終更新: 2026-05-24 / k-file v0.1.x (M5 完了 + M6a CLI 引数対応 + **M5b UX polish 完了**、β タグ前)

> **K-SystemZ 連携 API はこのドキュメント記載のまま変更なし** (CLI 引数仕様 §5.1
> も無改変)。M5b は k-file 本体の UI 磨き込み (フォント/複数選択/D&D 視覚化/
> プレビュー固定ヘッダー/Tab キー往復 等) で、K-SystemZ 側の改修依頼内容には
> 影響しない。読者 (K-SystemZ AI) は当該 §1-7 をそのまま実装してよい。
