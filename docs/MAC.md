...
>
> **✅ `v1.2.1` の手動導入は 2026-07-15 に Mac 実機で完了済** (起動成功 + 事件タブに
> 事件名表示を確認)。以後の Mac 更新はアプリ内バナー → [更新...] ボタンで済む。
> 次に新規 Mac へ入れるとき / 手動導入をやり直すときは下記コマンドを案内する
> (これが Mac 最後の手動更新の型)。**k-file を終了してから**実行:
>
> ```bash
...
**v1.2.1 の手動導入は 2026-07-15 に完了済** (起動 + 事件タブ表示を確認)。
以下は **次の版 (v1.2.2 以降) を出したときに確認する** (アプリ内自動更新の実機通し。未実施)。
