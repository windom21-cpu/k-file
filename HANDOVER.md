...
- **残タスク**: ①この修正を Win 実機で確認 (150%→100%→再起動で 100% に戻ること)。②Win 実機で 125〜200% のレイアウト崩れ (端の見切れ / ボタン内文字 / にじみ) を確認 (崩れたら該当固定 px を個別調整)。①②まとめて **v1.1.1 hotfix** としてタグ切り予定 (未タグ)。恒久の残課題はコード署名のみ。テスト 181→184 件 (`_apply_ui_scale` の env クリア回帰 3 追加。Win venv は symlink 2 件 skip、`test_ipc` 3 件は k-file 実機起動中だと単一インスタンスキー衝突で落ちる既知フレーク = 今回の変更と無関係)。
