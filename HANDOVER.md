...
## 現状サマリ
- 現在地: **M1 スケルトン完了 + UI/操作の設計詰め完了 (2026-05-22)。次は M2 実ファイル接続**
...
- **サブフォルダ内のネストフォルダは「階層タブ」で表示**: サブフォルダがフォルダを含むとき、ファイル一覧の上に横タブ帯を出す (`[○○直下] [子A] [子B]…`)。タブで横移動・パンくず (「事件フォルダ:」バーを拡張) で縦移動。何階層でも可。ツリービューではない (常に 1 フォルダのフラット一覧)。
...
- 独自ファイル形式の導入 (実ファイル主義)
- ツリービューでの事件フォルダ表示 (常に「サブフォルダボタン + フラット一覧」、ネストは階層タブ)
...
| Inbox → サブフォルダ投入 (マウス) | サブフォルダボタンへ D&D、または右クリック →「ここへ投入」 |
...
- **M2 実ファイル接続 + プレビュー**: Inbox 監視対象フォルダの実ファイル一覧 (QFileSystemWatcher)、事件フォルダの実ファイル読込 (QFileSystemModel)、PDF/画像プレビュー (QPdfView / QPixmap)、PDF + 画像フィルタ、無視機能、サブフォルダ動的読込 (Alt+1〜9 繰り上げ割当 + Alt+0 事件フォルダ直下)、ネストフォルダの階層タブ + パンくず
...
- `core/folder_scanner.py` — 事件フォルダの動的サブフォルダ取得 + `\d_.*` パターン + Alt+1〜9 繰り上げ割当 + ネストフォルダ走査
- `infra/kfile_db.py` 初版 — SQLite (kfile.db) スキーマ作成
- Inbox ペイン: ダミーデータを実ファイル一覧に差し替え (PDF + 画像のみフィルタ)
- 事件ペイン: ダミーデータを実フォルダ読込に差し替え (Alt+0 事件フォルダ直下ビュー + ネストフォルダの階層タブ)
