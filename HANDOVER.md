...
- **（前記録）`v1.1.0` 正式版を出荷 (2026-07-01)。** v1.0.0 実務安定運用後、2026-07-01 セッションで **表示倍率 (UI scale) 機能** を追加 → `v1.1.0` を正式リリース (prerelease=false / repo の latest、3 アセット = zip / .sha256 / k-file-setup.exe、CI test→build→Release 全緑)。**ウインドウ・文字・全 widget をパーセンテージで一律拡大縮小** (表示メニュー → 表示倍率: 75/100/125/150/175/200% ラジオ + 拡大 Ctrl++ / 縮小 Ctrl+- / 標準 Ctrl+0)。実装は Qt の `QT_SCALE_FACTOR` をデバイスピクセル段で効かせる方式 = QSS も固定 px も一切書き換えず Win95 レイアウトの px 不変条件 (バナー/バー offset ADR-42 等) を比率のまま保つ (`src/core/ui_scale.py` は Qt 非依存の純ロジック)。倍率は kfile.db `ui_scale_percent` に永続化、`main._apply_ui_scale` が QApplication 生成前に `QT_SCALE_FACTOR` へ反映。**反映は再起動** (QT_SCALE_FACTOR は Qt が起動時に一度だけ読むため。updater の relaunch を流用し「旧プロセス消滅待ち→起動」で IPC 競合を避けて自動再起動、タブ/ウインドウサイズも復元)、100% では env を未設定に戻す = OS 倍率 (ADR-45 PassThrough) に無影響 (ADR-46)。
- **⚠ v1.1.0 の表示倍率バグを `v1.1.1` で修正・出荷済 (2026-07-08、fix commit `1f0ecc8`)。** ユーザー報告「拡大表示は問題ないが、拡大した後 100% に戻して再起動しても 100% に戻らない」。**原因**: 自己再起動 (`_relaunch_app`) は現プロセスの環境を `subprocess.Popen(powershell)` → `Start-Process` 経由で新 exe へ継承する。150% 起動時に `_apply_ui_scale` が現プロセスに立てた `QT_SCALE_FACTOR=1.5` がそのまま子プロセスへ流れるが、新プロセスの `_apply_ui_scale` は「100% では env を触らない」設計だったため継承値 `1.5` が残り、db=100 でも 150% 表示のまま起動していた。**修正**: `_apply_ui_scale` で倍率読取後に必ず一度 `os.environ.pop("QT_SCALE_FACTOR")` してから、100% 以外のときだけ再セットする (DB 読取失敗の既定 100% 経路でも継承値を捨てる)。回帰テスト 3 件追加。**現行 v1.1.0 で 150% のまま固まっている場合の応急復帰**: メニューで 100% を選び直しても (db は既に 100 で `_effective_zoom` も 100 のため) 再起動が走らず直らない → **k-file を完全終了してショートカット/インストーラから新規起動** すればコールドスタートで継承 env が無く 100% で立ち上がる。修正版が Win に載れば以降は再起動でも正しく戻る。
- **Win 実機確認 完了 (2026-07-08 ユーザー確認)**: ①hotfix (150%→100%→再起動で 100% 復帰) ②125〜200% のレイアウト崩れ (端の見切れ / ボタン内文字 / にじみ) — いずれも問題なし。v1.1.2 hotfix 不要、**v1.1 系はクローズ**。恒久の残課題はコード署名のみ。テスト 181→184 件 (`_apply_ui_scale` の env クリア回帰 3 追加。Win venv は symlink 2 件 skip、`test_ipc` 3 件は k-file 実機起動中だと単一インスタンスキー衝突で落ちる既知フレーク = 今回の変更と無関係)。
- **（前記録）`v1.0.0` 正式版を出荷 (2026-06-21)。** β.11 まで実務で安定運用 → 2026-06-21 セッションで v1.0 配布固め 3 点を実装: ①更新 zip の SHA256 整合性検証 (CI が `.sha256` サイドカー出力 → 展開前に照合、不一致は fail-closed で中止、ADR-43) ②Inno Setup インストーラ (`installer/k-file.iss`、ユーザー領域 `%LOCALAPPDATA%\Programs\k-file` に管理者権限なしで導入 → 既存 zip 上書き型 自動更新と両立、ADR-44) ③High-DPI 追従 (`main.py` で PassThrough、ADR-45)。`v0.1.0-beta.12` を切って Win 実機で「インストーラ動作 / 125-150% 表示 / β.11→β.12 自動更新 (バナー + SHA256 照合 + 適用) 全工程」をユーザー確認 → 実務運用で問題なしと判断し 1〜2 週間並走を待たず **`v1.0.0`** を正式リリース (prerelease=false / repo の latest、3 アセット = zip / .sha256 / k-file-setup.exe)。**残る課題はコード署名のみ** (未署名 = 新規インストール時に Win SmartScreen「発行元不明」警告が残る、いつでも追加可)。テスト 152→162 件。
- 経緯 (β 期): **2026-06-05 セッション後半でレイアウト崩れ 2 件を根治し `v0.1.0-beta.8` を切った。①F4 半幅時に INBOX と参照フォルダで「更新列」が片方だけ消えて Name (ファイル名) 列幅が半々にならない現象 (両ペインが別々の viewport で 30px 閾値を独立判定 → 数 px 差が閾値をまたぐと片方だけ更新列が消える。Linux でも幅 831〜835px で再現。MainWindow が両テーブルの実 viewport の小さい方を共通基準に配って判定を一致させる、ADR-38) ②プレビュー表示中に「たまに INBOX だけ極端に狭くなる」現象 (情報行 QLabel が折返し無し = minimumSizeHint が全文幅 → 長いファイル名でプレビューペイン最小幅が 316→993px に膨張し splitter が preview を広げて inbox の幅を奪う。横 SizePolicy=Ignored + 右省略表示に変更、ADR-39)。直前の到達点は 🎉「自動アップデート機構 (案②) が実機で全工程動作」を β.5→β.6 で初確認 + 更新通知バナーの見切れ修正 (β.7、ADR-37)。なお β.1/β.2 同梱 updater は「再起動後無反応」、β.3/β.4 は「再起動後ハング」(cmd を DETACHED 起動すると `tasklist | findstr` パイプがデッドロック) を経て、updater を `cmd バッチ` → `PowerShell + CREATE_NO_WINDOW` に全面作り直し (commit `aa732dc`、ADR-36) で β.5 実機エンドツーエンド成功。③その後さらに更新通知バナーの「インフォメーションバー見切れ」を 2 段で根治: β.9 でバナーの font を 9pt に明示固定 (起動時 `apply_bitmap_font_strategy` が QSS の 9pt を継承 12pt で焼き直していた問題を回避) → それでも Win で文字/ボタンが見切れたため β.10 でステータスバー自体を 16→20px・バナーボタンを 14→18px に拡張 (ADR-40)。2026-06-08 セッションで **(a) ファイルのコピー/切り取り/貼り付け (Ctrl+C/X/V + 右クリック、Explorer 相互運用 CF_HDROP、ADR-41、Win 実機確認済) を追加**、**(b) バナー見切れの真因を特定して根治** — β.10 はバナーとバーを共に 20px に揃えたため `QStatusBar` の上寄せ offset(≈3px) 分が相殺されず**まだ +2px はみ出していた** (= β.10 でも見切れ継続)。真因は「バー実高 ≥ バナー高 + offset + slack」(ADR-42)。バナーを 16px に戻しバー 20px で overflow −2px に。**(c) CI の終了時 segfault を回避** — clipboard テストがグローバルクリップボードを触り offscreen Qt が終了時に落ちていた (mime レベルテストに変更)。これらを束ねて `v0.1.0-beta.11` をタグ切り + Release 公開 (CI: test→build→Release upload 全緑、prerelease + `k-file-windows.zip` 51MB)。現在地は `v0.1.0-beta.11`。次の能動アクションは β.11 への自動更新で コピペ機能 + 既存 3 件 (F4 半幅の列ズレ / 長いファイル名プレビューの inbox squeeze / バナー見切れ) を Win 実機確認 + 業務並走。⚠ **自己更新 UI の宿命**: 「直ったバナーが実際に出る様子」を Win で見るには β.11 を稼働中にさらに新版 (β.12) が要る (バナーは走っている版が描くため)。CI にテスト実行ゲートを追加 (ubuntu で offscreen pytest → 緑時のみ Windows ビルド/Release、`build: needs: test`、commit `9f45d6e`) + セキュリティ評価を実施 (詳細 §8 冒頭)。GitHub 2FA 済 (Authy) / CI テスト済、更新 zip の SHA256 照合・インストーラ/コード署名・High-DPI 確認は今後の課題**
  - M1〜M5b 完了 → 2026-05-25 1 回目 Win 機本番テスト → M5c で業務凍結級バグ + 設計修正 → 2026-05-26 2 回目検証 → M5d で消化 → 2026-05-27 M5e で β 直前 polish + 自動アップデート機構 (案②) → `v0.1.0-beta.1` タグ切り → 2026-05-29 M5f Phase 1 で preview 読込を別スレッド化 → 2026-06-01 M5f Phase 2① で再発フリーズ (一覧走査が主因と判明) に対し folder_scanner を scandir 化 → 2026-06-04 実機でフリーズ再現なし確認 + UI 操作性改善 (リサイズ/外枠/F4/Inbox ソート) → 2026-06-04 外枠を 2 段 4 色 raised に polish + `v0.1.0-beta.2` タグ切り + M5f Phase 3 掃除 (error.log 誤記録 / IPC 非同期化) → 2026-06-05 自動アップデート実機検証で updater のハング根治 (cmd→PowerShell 化、ADR-36) → β.3〜β.6 を順次切り、β.5→β.6 で**案②が実機で全工程動作することを初確認** → バナー見切れ修正で β.7 → 2026-06-05 レイアウト崩れ 2 件 (F4 半幅の更新列同期 ADR-38 / プレビュー情報行の幅暴走 ADR-39) を根治して β.8 → 更新通知バナーの見切れを font 明示 9pt (β.9) + バー高さ 16→20px (β.10) で根治 (ADR-40) → 2026-06-08 ファイル コピー/切り取り/貼り付け (Explorer 相互運用、ADR-41) + バナー見切れの真因 (バー高 ≥ バナー高 + QStatusBar offset。β.10 はまだ +2px はみ出していた) を根治 (ADR-42) + CI 終了時 segfault 回避 → `v0.1.0-beta.11`
  ...
  - **開発のメイン = Linux 本機 / 実際の挙動の修正 = Win 機** が原則 (2026-06-04 ユーザー確認)。実装・リファクタ・テスト・設計判断は Linux 本機で進めて push、Dropbox/X: フリーズ・フォント描画・リサイズの掴み心地など**実機でしか出ない挙動**の確認・修正は Win 機で行う (CLAUDE.md のワークフロー記述と一致)。※ M5f/M5g を Win 機中心で進めた時期に一時「本機 = Win」と記したが、原則は上記
...
| パッケージング | PyInstaller | `--onedir` + zip 配布 (M5c で確定) + Inno Setup インストーラ (ADR-44) |
| CI | GitHub Actions (windows-latest) | k-pdf3 と同じ流儀。test (ubuntu/offscreen) → build (needs: test) |
| 配布 | GitHub Releases | インストーラ + zip。**自動アップデート (案②、ADR-27) + SHA256 検証 (ADR-43)** 実装済 |
...
- **M5g UI 操作性 + 外枠 polish + Phase 3 + β.2 (✅ 2026-06-04 完了、`v0.1.0-beta.2` タグ切り)**: 全辺/全角リサイズ + Win95 風外枠 + F4 半幅 + Inbox 名前順 (M5g、ADR-32/33/34) → 外枠を 2 段 4 色 raised に作り直し (ADR-33 更新) → M5f Phase 3 掃除 (正常終了の error.log 誤記録停止 + IPC 受信非ブロック化、ADR-35) → M5f 根治 + M5g + 外枠を束ねて `v0.1.0-beta.2` prerelease。詳細 §7 (M5g/M5h)。テスト 102 → 138 (resize_grips/font_strategy/folder_scanner/preview/ipc 追加)
...
- **M7 v1.0 配布固め + 正式リリース (✅ 2026-06-21、`v1.0.0` 出荷)**: 更新 zip の SHA256 整合性検証 (ADR-43) + Inno Setup ユーザー領域インストーラ (ADR-44) + High-DPI PassThrough (ADR-45) を実装。`v0.1.0-beta.12` で Win 実機確認 (インストーラ動作 / 拡大率 125-150% / β.11→β.12 自動更新の全工程) → 実務運用で問題なしと判断し `v1.0.0` を正式リリース (prerelease=false、repo の latest)。残課題はコード署名のみ。テスト 152→162。
- **M8 表示倍率 (UI scale) + 正式リリース (✅ 2026-07-01、`v1.1.0` 出荷)**: ウインドウ・文字・全 widget をパーセンテージで一律拡大縮小する機能 (ADR-46)。Qt の `QT_SCALE_FACTOR` をデバイスピクセル段で効かせ QSS/固定 px を無改変のまま比率スケール、反映は updater relaunch 流用の自動再起動。表示メニュー → 表示倍率 (75/100/125/150/175/200% + Ctrl++/-/0)。`v1.1.0` を正式リリース (prerelease=false、repo の latest)。テスト 162→181。**残: Win 実機で 125〜200% のレイアウト崩れ確認** (崩れたら v1.1.1 hotfix)。**2026-07-08: 「100% に戻して再起動しても拡大が残る」バグ (自己再起動時の `QT_SCALE_FACTOR` 環境変数継承) を `_apply_ui_scale` で毎回 pop する形に修正 (commit `1f0ecc8`) → `v1.1.1` として正式リリース出荷。同日 Win 実機確認 (hotfix 復帰 + 125〜200% 崩れなし) 完了 → v1.1 系クローズ。詳細は現状サマリ冒頭を参照。**
- **M10 Mac 実務投入の詰め + 正式リリース (✅ 2026-07-15、`v1.2.1` 出荷)**: ①事件タブが空になるバグを修正 (macOS の NFD ファイル名、ADR-47) ②**Mac 用アプリ内自動アップデート** (.app 差し替え型 sh updater、ADR-48) — Phase 3 の積み残しを解消し、Mac も Win と同じワンクリック更新に。CI mac ジョブに SHA256 サイドカーを追加。テスト 188→204。**Mac は v1.2.1 だけ手動導入が必要** (更新機構を積んだ版自体は自動では入らないため。コマンドは現状サマリ冒頭)
- **M9 macOS (Apple Silicon) 対応 + 正式リリース (✅ 2026-07-10、`v1.2.0` 出荷)**: Phase 1 = CI `build-mac` + spec darwin 分岐で `k-file.app` 生成 → Phase 2 = Mac 実機チェック全項目クリア (メニューバー非表示バグ修正 `4b21694` のみ) → Phase 3 前半 = IPAゴシック同梱フォールバック (`src/ui/font_fallback.py`、システムフォント優先・欠落環境のみ代替、ライセンス全文同梱)。`v1.2.0` 正式リリース (latest、`k-file-macos.zip` 初同梱)。詳細 `docs/MAC.md`。残: Mac 用自動アップデート (シェルスクリプト版 updater) / コード署名 (Win)
...
- ✅ **Win 実機チェックは「クリア」とユーザー判断 (2026-06-04)**。四辺/四隅リサイズ・F4 半幅・
  Inbox 順は問題なし。外枠の見た目だけは下記 M5h で 2 段 4 色に作り直したので β.2 で再確認。

### M5h 外枠 polish + Phase 3 掃除 + `v0.1.0-beta.2` (2026-06-04 完了)

M5g の後、外枠の見た目を本物の Win95/98 窓枠に寄せ、ADR-29 で予告した Phase 3 掃除を
β.2 が Win で焼ける間に Linux 本機で前倒しし、M5f 根治 + M5g + 外枠を束ねて β.2 を切った。
**「開発 = Linux / 挙動修正 = Win」の原則どおりの分担**で進めたセッション。

#### ① 外枠を Win95/98 風 2 段 4 色 raised に作り直し (commit `00f159a`、ADR-33 更新)
- ✅ 初版 `_WindowFrame` は片側 1 色 (上左 #FFFFFF / 下右 #404040) の 2px ベタ塗りで「Win95 と
  違う感じ」とフィードバック。古典 3D システムカラーで「外側 1px + 内側 1px」の 2 段ベベルに:
  上/左 = #DFDFDF→#FFFFFF、下/右 = #000000→#808080。明 (上左) を先・暗 (下右) を後に塗り、
  右上/左下の隅は暗が勝つ = ボタンではなく「窓」に見える raised 陰影。
- ✅ `test_resize_grips.py` の bevel テストを 4 色 2 段に更新。コーナー拡大レンダリングで目視検証。

#### ② M5f Phase 3 掃除 (commit `6d5feb1`、ADR-35)
- ✅ `main.py`: `sys.exit()` を try の外へ。正常終了 (`SystemExit:0`) や secondary→primary 転送の
  `return 0` まで error.log に記録していた副次バグを停止 (実機 error.log の 4 件の痕跡を確認)。
- ✅ `ipc.py`: primary 受信を `waitForReadyRead/waitForDisconnected` の同期ブロック (最大 0.7 秒/
  接続) → `readyRead`/`disconnected` シグナルの非同期方式に。K-SystemZ 連打の累積ブロック対策。
- ✅ `tests/test_ipc.py` 4 件新規 (IPC は従来未テスト)。全 134 → 138 passed。実プロセス間でも
  secondary exit 0 + error.log の SystemExit 不増を確認。

#### ③ `v0.1.0-beta.2` タグ切り (commit `62c09c1`)
- ✅ M5f フリーズ根治 + M5g UI 操作性 + 外枠 polish を 1 本に束ねた prerelease。CI 成功で
  Releases に `k-file-windows.zip` (約 48.7MB) upload 済。**β.1 が Win 稼働中なら自動アップデート
  (案②) の本番初発火確認になる**。
- ⚠ **Phase 3 (②) は β.2 タグより後の commit `6d5feb1` なので β.2 には未同梱** — IPC/error.log
  の実機確認は β.3 を切ってから。
...
### 2026-07-27 セッション: F2 リネームの拡張子保護 → `v1.2.2` 正式リリース
...
- **`v1.2.2` を正式リリース**: bump `325d573` → tag `v1.2.2` → CI test→build→build-mac→Release
  全緑。アセット 5 点 (`k-file-windows.zip` / `.sha256` / `k-file-setup.exe` / `k-file-macos.zip` /
  `.sha256`)、prerelease=false / repo の latest。Win/Mac とも次回起動時に更新バナーが出る
- 環境メモ: このセッションは **WSL 側から Win venv (`.venv/Scripts/python.exe`) でテスト実行**。
  `test_font_fallback.py` の 1 件 (`test_no_substitution_when_preferred_exists`) はこの組合せだと
  変更前の main でも落ちる環境固有の失敗 (CI ubuntu では緑)

**残る課題** (前回から変わらず + 本件):
1. **Mac 実機で v1.2.1 → v1.2.2 のアプリ内自動更新を Phase 3 チェック** (`docs/MAC.md`) —
   Mac 自動更新の初の実機通し。ついでに F2 拡張子保護の動作も実機確認
...
**次の能動アクション (最優先)**: **Mac 機に v1.2.1 を手動で入れる案内を、セッション冒頭でユーザーに出す。**
コマンドは現状サマリ冒頭の 🔴 ブロックにコピペ完成形で置いてある。これが「最後の 1 回の手動更新」で、
以後 Mac もアプリ内のボタン 1 つで更新できる。入れたら `docs/MAC.md` の **Phase 3 チェックリスト**
(自動更新の実機確認) を次の版 (v1.2.2 以降) を出したときに回してもらう。

**この日やったこと** (すべて Linux 側セッション。Mac 実機確認は未実施 = 次回):

- **① 事件タブの文字が出ない (Mac) を修正** (`5b13d35`、ADR-47)
  - ユーザー報告「mac で開くとタブの文字が表示されず、どの事件ファイルを開いているか分からない」
  - **フォント/描画の問題ではなかった**。Mac の readdir が日本語名を **NFD (濁点分解)** で返すため、
    `_parse_case` の NFC リテラル `"文書フォルダ"` と一致せず解析が丸ごと失敗 → 表示名が空文字 →
    「枠だけで文字のないタブ」。切り分けはユーザーに **パスバーの表示**を聞いて確定
    (フォルダ名が丸ごと出る = 解析失敗の裏付け。フォント問題なら他の文字も崩れるはず、が崩れていない)
  - 修正: 照合前に `unicodedata.normalize("NFC", path.name)`。同種の罠がある
    `case_repo.resolve_folder` も NFC 揃え。`_tab_label` で「解析失敗でも空タブにしない」防御を追加
    (命名規則から外れたフォルダで **Win でも空タブになり得た** 潜在バグも封鎖)
  - 既知の副作用: **Mac で記録済の F12 履歴は case_code 欄にフォルダ名全体が入っている**
    (今後の記録は正しい)。履歴からの事件フォルダ復帰は前方一致なので実害なし、表示が長いだけ
- **② Mac 用アプリ内自動アップデート** (`7f24f30`、ADR-48) — Phase 3 の積み残しを解消
  - ユーザー要望「手動更新は構わないが、せめてアプリ内でボタン一つで更新したい」
  - フローは Win と同一、実行系だけ差し替え: sh スクリプト + `ditto -x -k` で `.app` まるごと差し替え
    (`write_mac_updater_script`)。PID 消滅待ち → 展開 → 旧 `.app` を `.old` 退避 → 設置 → `open`、
    失敗時はロールバックして旧版を起動。`~/.config/k-file/updates/updater.log` に経緯
  - **Gatekeeper 警告は出ない** (検疫マークはブラウザ DL 品に付く印。アプリ内 DL には付かない)
    = 更新のたびの `xattr -cr` が不要になる
  - Release に Win/Mac 両 zip が載るので asset は OS で選び分け、fallback でも他 OS のものを掴まない
  - CI mac ジョブに `k-file-macos.zip.sha256` を追加 (整合性検証が Mac でも効く)
  - 副産物: **Mac の表示倍率変更後の自動再起動も修復** (従来は PowerShell を呼んで失敗し警告)
  - 検証: Mac 実機が無いため、**生成した sh を Linux 上で実際に実行** して確認
    (`tests/test_updater_mac.py`、ditto/open/xattr を偽コマンドに差し替え)。テスト 188→204
- **`v1.2.1` を正式リリース**: bump → tag `v1.2.1` → CI test→build→build-mac→Release。
  アセット 5 点 (`k-file-windows.zip` / `.sha256` / `k-file-setup.exe` / `k-file-macos.zip` /
  **`k-file-macos.zip.sha256` を初同梱**)。Win 機は v1.2.0 の自動更新バナーから v1.2.1 へ

**残る課題**:
1. **Mac 実機での確認 (次回)** — v1.2.1 手動導入 → タブに事件名が出るか → 次版で自動更新の Phase 3 チェック
2. **コード署名 (Windows)** — 恒久の残課題 (v1.0 から継続)
3. 改善候補: `test_ipc` の実機衝突分離 / F12 投入履歴のサムネ版 / .icns アプリアイコン

---

### 2026-07-10 セッション: macOS 対応 Phase 1〜3 (フォント) → `v1.2.0` 正式リリース

**この日やったこと** (Phase 1 は Linux 側、Phase 2 以降は Mac 実機セッション):
- **Phase 1 (Linux 側)**: CI に `build-mac` ジョブ (macos-latest/arm64、`needs: test`) + spec の
  darwin 分岐 (.app BUNDLE、NSHighResolutionCapable)、Mac では自動更新チェック無効化
  (updater が PowerShell 前提のため)、`docs/MAC.md` 新設 (`70c0fc3`)
- **Phase 2 (Mac 実機、事務所 Mac / Apple Silicon)**: チェックリスト**全項目クリア** (ユーザー確認)。
  結果詳細は `docs/MAC.md` に記録。要点:
  - 見つけて直したバグは 1 件のみ: **ウインドウ内メニューバーが macOS で非表示** (`4b21694`)。
    親なし `QMenuBar()` は cocoa でグローバルメニューバー扱いになり、一度 native 化すると
    `setNativeMenuBar(False)` を後から呼んでもレイアウトに乗らず非表示のまま。
    生成時に親 widget を渡して回避 (Windows/Linux 無影響)
  - この Mac は **Office 由来の MS Gothic / MS UI Gothic が実在** (QFontDatabase で確認) =
    表示崩れが無かった理由。frameless リサイズ / 投入 (Option+数字, D&D, 右クリック) / Undo /
    F12 / Del→Finder ゴミ箱 / プレビュー / コピー→Finder 貼り付け すべて問題なし
  - **Win でフリーズ前科 (ADR-29/30) のフォルダを開いてプレビューしても発生せず** (ユーザー確認)。
    オンラインのみファイルの DL 遅延だけ業務並走で継続観察
  - ksystemz.db は `~/Library/CloudStorage/Dropbox/K-system/ksystemz/ksystemz.db`、
    `office_info.doc_root_path_mac` が実パスと一致することを sqlite RO で確認済
- **Phase 3 前半 = IPAゴシック同梱フォールバック (`856cbd8`)**: **ユーザー決定 (2026-07-10) で
  フォント方針を一部更新 — 「常時 IPA」ではなく「システムに MS Gothic があればそれを優先、
  無い環境だけ同梱 IPAゴシックへ代替」**。`src/ui/font_fallback.py` の `ensure_gothic_fallback`
  (QFontDatabase 判定 → `addApplicationFont` + `QFont.insertSubstitution`、main.py が QSS 適用前に
  呼ぶ)。`resources/fonts/` に ipag.ttf (v003.03) + IPA フォントライセンス v1.0 全文を同梱
  (再配布条件)、spec の datas に追加。テスト 4 件追加 (184→188、両経路を monkeypatch で強制)
- **`v1.2.0` を正式リリース**: bump `7cb5b26` → tag `v1.2.0` → CI test→build→build-mac→Release
  全緑。prerelease=false / repo の latest、**アセット 4 点 (`k-file-macos.zip` を Release に初同梱)**。
  Win 機は v1.1.1 の自動更新バナーから v1.2.0 へ
- **Mac 実機の開発環境を構築**: `~/Desktop/k-file` (clone + `.venv`、Python 3.14 + PySide6 6.11.1)。
  テスト 188 全緑 (Win の symlink skip 2 件も Mac では走る)。ソース起動は
  `.venv/bin/python -m src.main`。ビルド版は `~/Applications/k-file.app` (v1.2.0 リリース版を
  `xattr -cr` で検疫解除して設置)。**Mac は自動更新が無いため、新版は Release の
  `k-file-macos.zip` を手動 DL → 差し替え → `xattr -cr`** (Phase 3 後半で解消予定)

**残る課題**:
1. **Mac 用自動アップデート** (Phase 3 残り) — PowerShell updater のシェルスクリプト +
   .app 差し替え版への書き直しが最大の作業 (2026-07-08 見積どおり)
2. **コード署名 (Windows)** — 恒久の残課題 (v1.0 から継続)
3. 改善候補: `test_ipc` の実機衝突分離 / F12 投入履歴のサムネ版 / .icns アプリアイコン

**次の能動アクション**: `v1.2.0` で Mac + Win 業務並走。Mac で要望・不具合が出たら
Mac 実機セッションでも「修正 → push → CI → リリース」を一通り回せる (今回のセッションで実証済)。

---

### 2026-07-08 セッション: `v1.1.1` hotfix 出荷 + Win 実機確認完了 + Mac 対応方針の検討

**この日やったこと**:
- **`v1.1.1` hotfix を出荷** (詳細は現状サマリ冒頭)。v1.1.0 の「拡大後 100% に戻して再起動しても
  拡大が残る」バグ (自己再起動の `QT_SCALE_FACTOR` env 継承) を `_apply_ui_scale` の毎回 pop で修正
  (commit `1f0ecc8`)。回帰テスト 3 件追加 (181→184)。
- **Win 実機確認を完了 (ユーザー確認)**: ①hotfix (150%→100%→再起動で 100% 復帰) ②125〜200% の
  レイアウト崩れ — いずれも問題なし。v1.1.2 不要、**v1.1 系はクローズ**。
- **将来の Mac 対応の方向性を検討・確定** (着手時期は未定、結論のみ確定):
  - **方式は mac ビルド (PySide6 クロスビルド) 一択。WEB 版ファイラー衛星は不採用** —
    ブラウザはローカルファイル操作 (フォルダ監視 / 移動 / 既定アプリ起動 / ごみ箱) が原理的に
    できず、実用にはローカル常駐エージェントが必要 = 二重開発で本末転倒。守秘 (ローカル完結原則)
    とも相反し、F12/Ctrl+W 等キーボード体系もブラウザと衝突するため。
  - **繋ぎは Parallels 等の Win 仮想環境** (開発ゼロで現行 Win 版がそのまま動く)。Mac が主力と
    確定した時点でビルド着手。
  - **自分専用なら Apple 署名・公証 (年 $99) は不要**: Gatekeeper の審査は DL 品の初回起動のみ。
    初回だけ「このまま開く」(or `xattr -d com.apple.quarantine`)。アプリ自身が DL する自動更新
    zip には検疫マークが付かないため、以後の自動更新は警告なしの見込み。事務所内配布を始める
    段になったら課金検討。
  - **フォント方針 (ユーザー決定)**: PixelMplus 等ドット絵風フォントの同梱は「ガタガタしすぎて
    極端」なので**避ける**。第一候補 = **IPAゴシック同梱** (MS Gothic 寸法互換で px レイアウト
    見切れリスク最小 / IPA フォントライセンスで同梱再配布可 / Win 版「なめらか」モードに近い顔)
    + Mac では描画モード「中間/なめらか」既定 (「ガタガタ」はビットマップ字形が無く成立しない
    ため無効化)。ヒラギノは異常時フォールバックの保険のみ (モダンな顔 + 寸法差で主役にしない)。
    レトロ感は beveled 外枠・灰色・高密度側で維持。
  - **着手時の主作業 (見込み順)**: ①updater の Mac 対応 (PowerShell → シェルスクリプト +
    .app 差し替え) が最大 ②Finder 連携 (CF_HDROP コピペ `ui/clipboard_ops.py` / Explorer で開く /
    ごみ箱 `infra/recycle_bin.py`) ③フォント同梱 (上記)。パス解決は `case_repo.py` が
    `doc_root_path_mac` 対応済で土台あり。X: は Mac に無いので Inbox 等のパス設定は Mac 用に
    持ち直し (設定ダイアログで吸収)。

**残る課題**:
1. **コード署名 (Windows)** — 恒久の残課題 (v1.0 から継続)。
2. 改善候補: `test_ipc` の実機衝突分離 / F12 投入履歴のサムネ版。差分更新 (案③) は当面保留。

**次の能動アクション**: `v1.1.1` のまま業務並走。要望・不具合が出たら同じ流れ
(実装 → push → CI → 自動更新) で対応。Mac 対応は必要性が確定した時点で上記方針から着手。

---

### 2026-07-01 セッション: 表示倍率 (UI scale) → `v1.1.0` 正式リリース

**この日やったこと**:
- **表示倍率 (UI scale) 機能を実装** (ADR-46)。ウインドウ・文字・全 widget をパーセンテージで
  一律拡大縮小。Qt の `QT_SCALE_FACTOR` をデバイスピクセル段で効かせる方式 = QSS も固定 px も
  書き換えず、Win95 レイアウトの px 不変条件 (ADR-42 等) を比率のまま保つ。
  - `src/core/ui_scale.py` (新規、Qt 非依存): 倍率の正規化 (clamp/parse) + 段階移動 (step_up/down)
    + `scale_factor_str` (QT_SCALE_FACTOR 用文字列)。SETTING_KEY=`ui_scale_percent`、SCALE_STEPS=
    75/100/125/150/175/200。
  - `main._apply_ui_scale`: QApplication 生成前に kfile.db の倍率を読み `os.environ["QT_SCALE_FACTOR"]`
    へ反映。100% では env に不干渉 (= OS 倍率 ADR-45 に無影響)。
  - 表示メニュー → 表示倍率: ラジオ (75〜200%) + 拡大 Ctrl++ / 縮小 Ctrl+- / 標準 Ctrl+0。
    倍率変更時は現状と違えば再起動を促し、承認で自動再起動 (`_relaunch_app`)。
  - `updater.write_relaunch_script`: 更新の relaunch を流用した「旧プロセス消滅待ち→起動」だけの
    PowerShell (zip 展開なし)。IPC 単一インスタンスロック解放を待ってから起動するので競合しない。
  - 高倍率で復元ウインドウが画面外に出ないよう `_restore_window_size` を availableGeometry でクランプ。
  - テスト: `tests/test_ui_scale.py` 19 件追加 (162→181)。
- `v1.1.0` を **正式版** として push + タグ → CI test→build→Release 全緑、prerelease=false / repo の
  latest / 3 アセット。稼働中の v1.0.0 は起動時に v1.1.0 を検知して更新バナーを出す。

**残る課題 (v1.1 以降)**:
1. **Win 実機で 125〜200% のレイアウト崩れ確認** (今回の最優先の検証)。ADR-45 で「拡大時の見え方は
   実機要確認」とされていた唯一の残リスク。端の見切れ / ボタン内文字の収まり / にじみを各倍率で確認し、
   崩れたら該当固定 px を個別調整して v1.1.1 hotfix。※ビットマップ字形は 100% で最もシャープ、非 100%
   では中間/なめらか寄りに滲む (「表示 → 文字の描画」で切替可)。
2. **コード署名** (恒久の残課題、v1.0 から継続)。未署名 = 新規インストール時に Win SmartScreen 警告。
3. 改善候補: `test_ipc` の実機衝突分離 / F12 投入履歴のサムネ版。差分更新 (案③) は当面保留。

**次の能動アクション**: Win 実機で表示倍率 (125〜200%) の崩れを確認。問題があれば固定 px 調整で
v1.1.1、無ければそのまま業務並走。

---

### 2026-06-21 セッション: v1.0 配布固め → `v1.0.0` 正式リリース

**この日やったこと**:
- 更新 zip の SHA256 整合性検証を実装 (ADR-43)。CI が `k-file-windows.zip.sha256` (sha256sum 形式)
  を出力 → `update_banner._verify_checksum` が DL 後・展開前に照合、不一致は fail-closed で適用中止。
- Inno Setup インストーラ (`installer/k-file.iss`) を新規追加 (ADR-44)。ユーザー領域
  (`%LOCALAPPDATA%\Programs\k-file`) に管理者権限なしで導入 → 既存 zip 上書き型 自動更新と両立。
  CI が choco で Inno を入れ ISCC でビルド、`k-file-setup.exe` を Release/artifact に追加。
- High-DPI 追従 (ADR-45)。`main.py` で `HighDpiScaleFactorRoundingPolicy=PassThrough`。
- CI バグ修正: VERSION 抽出を行頭アンカー化 (docstring 内の例行を誤検出していた)。
- `v0.1.0-beta.12` を切って Win 実機で全確認 (インストーラ動作 / 125-150% 表示 / β.11→β.12
  自動更新の全工程 = バナー + SHA256 照合 + 適用) → いずれも OK。
- 実務運用で問題なしとのユーザー判断で 1〜2 週間並走を待たず **`v1.0.0`** を正式リリース
  (prerelease=false / repo の latest、3 アセット添付)。テスト 152→162 件。

**残る課題 (v1.0 以降)**:
1. **コード署名** (唯一の残課題)。未署名のため新規インストール時に Win SmartScreen「発行元不明」
   警告が出る (zip より .exe インストーラの方が強く出る)。有料証明書 (年 ¥1〜4 万) の費用対効果で
   判断。署名すれば警告解消 + 自動更新経路 (唯一の遠隔攻撃面) の本命防御にもなる (SHA256 では塞げない)。
2. 改善候補: `test_ipc` の実機衝突分離 / F12 投入履歴のサムネ版。差分更新 (案③) は当面保留。

**次の能動アクション**: `v1.0.0` を業務並走で使い込み、要望・不具合が出たら同じ流れ
(実装 → push → CI → 自動更新) で対応。大きな機能追加が要れば M8 として起票する。

---

### 2026-06-05 セッション成果 + 今後の課題 (セキュリティ / 配布 / 改善)

**この日やったこと**:
- レイアウト崩れ 2 件根治 → β.8 (F4 半幅の更新列同期 ADR-38 / プレビュー情報行の幅暴走 ADR-39)
- 更新通知バナーの「インフォメーションバー見切れ」根治 → β.9 (font 明示 9pt) + β.10
  (ステータスバー 16→20px / バナーボタン 14→18px、ADR-40)
- **CI にテスト実行ゲート追加** (commit `9f45d6e`、上記 §15 末の記録参照)。
- セキュリティ評価を実施 (法律実務=守秘の観点)。**良い点**: ローカル完結で外部送信なし /
  ksystemz は RO / `shell=True`・`os.system`・`eval`・`pickle` 不使用 (外部コマンドは全て list 形式
  = コマンド注入面なし) / 通信は TLS 検証あり。**唯一の遠隔攻撃面 = 自動アップデート経路**
  (乗っ取られると依頼者ファイルを持つ PC で任意コード実行になり得る)。

**今後の課題 (優先順)**:
1. ~~GitHub アカウント 2FA~~ → **済み (Authy、ユーザー確認 2026-06-05)**。自動更新の信頼の根。
   補足: GitHub リカバリーコードを Authy とは別に保管 / フィッシング (偽ログイン画面) に注意。
2. ~~CI でテスト実行~~ → **済み (今セッション)**。
3. **更新 zip の完全性検証 (SHA256、多層防御の保険)**: CI で zip の SHA256 を計算して Release
   本文に載せ、アプリ側 (`update_banner._on_dl_finished` の展開前) で照合する。未着手。
4. **配布の本格化 (M6 / v1.0 に向けて)**: インストーラ化 (Inno Setup 等、案: 現 zip 上書き →
   installer silent 実行、メモ `project_install_update_direction`)、**コード署名** (未署名のため
   Win SmartScreen が「発行元不明」警告。証明書は有料 ≒ 年 1〜4 万円で費用対効果判断)、
   Explorer 右クリック「k-file で開く」シェル拡張。
5. **改善候補**: High-DPI / 画面スケーリング (125%/150%) 表示の確認 (ビットマップ font + 固定 px
   前提のため崩れる恐れ。今回の F4/列幅バグもピクセル手調整起因)、`test_ipc` の実機衝突分離、
   F12 投入履歴のサムネ版。

**次の能動アクション**: `v0.1.0-beta.11` (= コピペ機能 ADR-41 + バナー見切れ真因根治 ADR-42 +
既存の F4 列ズレ/inbox squeeze/バナー を同梱、Release 公開済) を業務並走で使い込み、業務凍結級
バグが出なければ v1.0 stable へ。とくに **コピペ (Ctrl+C/X/V + 右クリック、Explorer 双方向)** を
実業務で叩く + **バナーがバー内に収まっているか**を Win 実機で確認 (ただし「直ったバナーが出る様子」を
見るには β.11 稼働中に β.12 が要る ＝ 自己更新 UI の宿命)。上記 3〜5 は v1.0 までの課題として随時着手。
...
3. ~~**Phase 3 (任意の掃除)**~~ → **✅ 完了 (2026-06-04、commit `6d5feb1`、ADR-35、M5h②)**。
   IPC の同期待ち (`ipc.py` の `waitForReadyRead(500)`+`waitForDisconnected(200)`、K-SystemZ
   連打で累積) を `readyRead`/`disconnected` シグナルの非同期方式に + `main.py` の
   `except BaseException` が正常終了 (`SystemExit:0`) まで error.log に書く副次バグを修正
   (`sys.exit()` を try 外へ)。`tests/test_ipc.py` 4 件追加。⚠ β.2 より後の commit なので
   Win での効果確認は β.3 から。
...
2. ~~**β.1 で問題が出れば β.2**~~ → **✅ `v0.1.0-beta.2` 切り済 (2026-06-04、commit `62c09c1`)**。
   M5f 根治 + M5g + 外枠 polish を束ねた prerelease (β.1 のバグ待ちではなく、根治分を早く
   届けるため積極的に切った)。**β.1 が Win 稼働中なら、起動時に「🔔 v0.1.0-beta.2 が公開
   されました [更新...]」→ 自動 DL → 再起動 → β.2、が自動アップデート機構 (案②) の本番初
   発火確認になる** ← 次の能動アクション。Phase 3 掃除 (error.log/IPC、commit `6d5feb1`) は
   β.2 より後なので未同梱、必要なら β.3 で束ねる。
3. **β → v1.0 stable (M6)**: β.2 を 1〜2 週間 Win で業務並走し重大バグが出なければ
   `VERSION="1.0.0"`。stable 前提 (docs/VERSION-BUMPING.md): 診断コード撤去 (= Phase 3 で
   前倒し済) / installer 化 / icon.ico / code signing。**将来方針「インストーラ導入 + アプリ内
   アップデート」= 現 zip-in-place 自動更新 (案②) を installer silent 実行型に作り直す**分岐が
   ここで効く (ユーザー希望 2026-06-04)。
...
- **決定**: ウインドウ全体を覆う透明オーバーレイ widget (`_WindowFrame`) を最前面に置き、
  `paintEvent` で Win95/98 風の **2 段 4 色 raised 外枠**だけを描く。中身は透過、
  `WA_TransparentForMouseEvents` でマウスも素通り = リサイズグリップや操作を一切妨げない。
  `ResizeGrips.reposition` でグリップと一緒に縁へ追従し、最大化中も枠は表示維持。
- **色 (2026-06-04 polish、commit `00f159a`)**: 初版は片側 1 色 (上/左 #FFFFFF・下/右 #404040) の
  2px ベタ塗りで「Win95 と違う感じ」とフィードバック。本物の窓枠は「外側 1px + 内側 1px」の
  2 段ベベルなので、古典 3D システムカラーで作り直した: **上/左 = 外側 #DFDFDF (3DLIGHT) →
  内側 #FFFFFF (白)、下/右 = 外側 #000000 (3DDKSHADOW) → 内側 #808080 (影灰)**。明 (上左) を
  先・暗 (下右) を後に塗るので右上/左下の隅は暗が勝ち、ボタンではなく「窓」に見える raised
  陰影になる。`#C0C0C0` 背景に直接この 4 色エッジを載せるのが Win95 窓枠の最小十分構成
  (灰色サイズ枠は背景と同化するので省略)。
- **High-DPI 対応**: `drawLine` は DPR=2 で端の device pixel を 1px 取りこぼすため、
  各リングを **`fillRect` で 1px 帯塗り** (論理矩形を device pixel まで正確に塗る)。pixmap grab で
  「外側 3DLIGHT→内側白 / 下右 外側黒→内側影灰 / 右上・左下隅は暗が勝つ raised 陰影」を検証済
  (`test_resize_grips.py` の bevel テストを 4 色 2 段に更新)。
...
### ADR-35: 正常終了を error.log に記録しない + IPC 受信をメインスレッドでブロックしない (M5f Phase 3) (2026-06-04)
- **経緯**: ADR-29 で予告した「Phase 3 (任意の掃除)」を、β.2 が Win で焼ける間に Linux 本機で
  前倒し。① `main.py` の `except BaseException` が**正常終了まで** error.log に書く副次バグ、
  ② IPC primary 受信の同期ブロック の 2 点。stable 移行の前提「β 診断コードの撤去」(docs/
  VERSION-BUMPING.md) の一部でもある。
- **① 正常終了の誤記録**: `sys.exit(main())` を try 内に置いていたため、`main()` が返した
  終了コードでの `SystemExit` (正常終了 = `SystemExit:0`、secondary→primary 転送の `return 0`
  含む) まで `except BaseException` が拾い、毎回 error.log に SystemExit のトレースを記録して
  いた (実機 error.log に 4 件の `SystemExit:0` 痕跡を確認)。"困ったら error.log を見る" 運用が
  ノイズで機能しなくなる。**決定**: `sys.exit()` を try の外へ出し、`main()` 実行中に実際に
  送出された致命例外だけを記録する。
- **② IPC 同期ブロック**: `IpcServer._on_new_connection` が `waitForReadyRead(500)` +
  `waitForDisconnected(200)` でメインスレッドを最大 0.7 秒/接続ブロックしており、K-SystemZ
  から「フォルダを開く」を連打されると累積。**決定**: `readyRead`/`disconnected` シグナルで
  受け取る非同期方式に変更し、相手が切断しないまま放置/crash した場合だけ 1 秒タイマーで
  打ち切る。newConnection 処理時に既に切断済みのケースも明示回収。parse は `_parse_paths` に
  抽出。secondary 側 (`try_send_to_primary`) は送信後すぐ exit する ephemeral プロセスなので
  同期のまま据置 (UI を持たない = ブロックしても無害)。
- **テスト**: `tests/test_ipc.py` 4 件新規 (これまで IPC は未テスト): `_parse_paths` / primary
  不在時 False / ラウンドトリップ (日本語パス) / 空送信でも callback 発火。全 138 passed。実
...
### ADR-40: 更新通知バナーの見切れは「font 明示 9pt + バー高さ 20px」で根治する (ADR-37 の完了) (2026-06-05)
- **症状**: 右下インフォメーションバーの更新通知 (`UpdateBanner`) が、文字もボタンも常に縦に見切れる。
  ADR-37 (β.7) で QSS に `font-size:9pt` を入れたが Win で直っていなかった。
- **原因 (2 層)**:
  1. **font が 12pt に焼き直される**: 起動時 `apply_bitmap_font_strategy` が全 widget を walk して
     `w.font()` を `setFont` し直す。QSS の `font-size` は `w.font()` に反映されない (描画段のみ) ため、
     バナーは継承した 12pt を「明示フォント」として焼き直され、QSS の 9pt を上書きしていた。
  2. **バー高さ不足**: 9pt の字 (Win の MS Gothic で ~13-16px) + Win95 のベベル枠 1px*2 は、
     16px のステータスバー / 14px のバナーボタンに物理的に入りきらない。
- **決定**:
  1. `UpdateBanner.__init__` で label / 更新 / × の各 widget の font を 9pt に**明示設定**。walk は
     `point_size=None` で既存サイズを保つため 9pt が維持される (β.9)。
  2. `QStatusBar` を 16→**20px**、`#updateBannerBtn` を 14→**18px** (内寸 16px)、`UpdateBanner` を
     `setFixedHeight(20)` に拡張 (β.10)。オフスクリーン grab で更新/× ボタンの枠と文字が縦に切れず
     収まることを確認。
- **積み残し/注意**: 自己更新 UI の宿命で、直ったバナー自体を Win で見るには β.10 稼働中にさらに
  新版が要る (バナーは走っている版が描く)。バー高さの 20px 化は β.10 起動時点で即確認可。

### CI にテスト実行ゲートを追加 (2026-06-05、commit `9f45d6e`)
- それまで CI (`build.yml`) は Windows で .exe をビルドするだけで **144 件のテストを実行して
  いなかった** = 壊れたコミットがそのまま .exe 化され、自動更新で全端末に配布され得る穴。
- `test` ジョブ (ubuntu-latest / `QT_QPA_PLATFORM=offscreen` で pytest) を新設し、`build` を
  `needs: test` に。**テストが緑の時だけ Windows ビルド/Release に進む**。タグ (リリース) でも
  test→build 順なので、テストが落ちる版は自動的に配布されない。
- PySide6 (pip 版) は libEGL/libGL を同梱しないため `libegl1 libgl1 libxkbcommon0 libdbus-1-3`
  を apt install。CI 実機で `144 passed in ~8s`、build も成功を確認。ADR には未昇格 (設計判断
  というよりプロセス改善のため、§8 冒頭に課題と並べて記録)。
- **2026-06-08 追記 (終了時 segfault のゲート発火)**: `test_clipboard_ops.py` 追加後、CI の test
  ジョブが「151 passed」直後に **Segmentation fault (exit 139)** で落ちた。原因はテストが
  グローバル `QApplication.clipboard()` に `QMimeData` を載せ、Linux/offscreen の Qt が
  インタプリタ終了時 (静的デストラクタ) のクリップボード破棄で segfault するため。既存スイートは
  クリップボードを一切触っていなかったので顕在化していなかった。対処は ADR-41 末尾 (mime レベル
  テスト) 参照。**教訓: offscreen の自動テストでグローバルクリップボードを触らない**。テストゲートが
  この穴を実際に止めた = ゲート導入の有効性が確認できた事例。

### ADR-41: ファイルのコピー/切り取り/貼り付けは OS クリップボード (CF_HDROP) で行う (2026-06-08)
- **背景**: Inbox / 参照フォルダで Explorer 流のコピペが欲しい (案件文脈で Explorer を開かずに
  済ませる方針 §1 の延長)。ユーザーは「k-file ⇄ Explorer の双方向」を希望。
- **決定**: `src/ui/clipboard_ops.py` を新設。Windows シェルと同じクリップボード形式で載せる:
  - ファイル実体は `text/uri-list` (= Win では CF_HDROP) に URL として載せる (Qt が相互変換するため
    Explorer ⇄ k-file 双方向でコピペが効く)。
  - コピー/切り取りの別は Windows 独自の `Preferred DropEffect` 形式 (4 byte DWORD: COPY=1 / MOVE=2)
    で伝える。これを書けば Explorer の Ctrl+V がコピー/移動を正しく選び、読めば Explorer 側が
    Ctrl+C/Ctrl+X どちらをしたか分かる。
  - 操作: `Ctrl+C / Ctrl+X / Ctrl+V` (`QKeySequence.StandardKey`、両ペインの table に
    `WidgetWithChildrenShortcut` で結線) + 右クリック「コピー / 切り取り / 貼り付け」。余白の
    右クリックでも「貼り付け」を出す。既存の「フルパスをコピー」(テキスト) はそのまま別物で残置。
  - 貼り付け先: 参照フォルダ = `case_pane.current_dir()` (表示中フォルダ)、Inbox = 選択フィルタタブの
    ソースフォルダ (「全て」タブで複数ソース時は選択行の出所、無ければ無効)。
  - 実コピー/移動は MainWindow `_on_paste` に集約 (パネルは `pasteRequested(str)` を emit)。
    コピー=`file_ops.copy` / 切り取り=`file_ops.move`、衝突は自動連番、`drop_history` 記録で
    `Ctrl+Z`/F12 から Undo 可 (undo_ops は既に copy/move 対応)。切り取り→貼り付けは一度きり
    (成功後にクリップボードを空に = Explorer と同じ)。同一フォルダへの切り取り貼り付けは no-op。
  - **テストは mime レベル**: グローバルクリップボードを触ると offscreen Qt が終了時 segfault する
    (上記 CI 追記) ため、`build_file_mime` / `parse_file_mime` を純粋関数に切り出し、ローカル
    `QMimeData` を直接往復させて検証 (`test_clipboard_ops.py` 8 件)。実クリップボードと Explorer
    相互運用は Win 実機で確認済 (2026-06-08、ユーザー確認)。

### ADR-42: 更新通知バナー見切れの真因は「バー実高 ≥ バナー高 + QStatusBar の上寄せ offset」(ADR-40 の幾何を訂正) (2026-06-08)
- **発覚**: ADR-40 (β.10) で「バー 20px + バナー 20px + ボタン 18px」にしたが、**offscreen 実測で
  まだ下端が +2px はみ出していた** (= β.10 をビルドしてもバナーは見切れたまま)。β.4〜β.10 で
  毎回バナー/ボタン/font を弄っても再発し続けた真因はここ。
- **真因**: `QStatusBar` は `addPermanentWidget` した子を内部レイアウトで**上端から y≈3px に上寄せ
  固定**で置く (中央揃えではない)。`UpdateBanner` は `setFixedHeight` の固定高さ widget なので、
  見切れない条件は次の不等式:
  ```
  バー実高 (= QSS height + border 1px) ≥ バナー高 + 3px(offset) + slack
  ```
  β.10 はバナーとバーを共に 20px に揃えたため offset 分が相殺されず、バナー下端 (y3+20=23) が
  実バー 21px を 2px 超過していた。**「バナーを縮める対症療法」ではなく「バー側に余白を持たせる」**
  のが根治。
- **決定**: バナーを 16px に戻し、ボタンを内寸 14px(+枠2px=16px)、バーは 20px (実 21px) を維持。
  banner 16 @ y3 → 下端 19 ≤ 21 で **overflow −2px** (offscreen 実測 + grab で枠が上下とも欠けない
  ことを確認)。β.10 が正しく入れた「font を 9pt に明示固定」(ADR-40 の原因①対策) はそのまま維持。
  `win95.qss` の `QStatusBar` にこの不変条件をコメントで明記し再発防止 (バナー高を変えるなら
  必ず不等式を満たすこと)。
- **教訓**: ステータスバー常駐 widget の縦見切れは、widget 自身の高さではなく「コンテナの実高 −
  コンテナ内部 offset」で決まる。固定高さ widget では font metrics は無関係 (高さは確定値)。

### ADR-43: 更新 zip の整合性は SHA256 サイドカー照合で担保 (本命脅威には弱い保険、2026-06-21)
- **背景**: 自動アップデート経路は唯一の遠隔攻撃面 (乗っ取られると依頼者ファイルを持つ PC で任意
  コード実行になり得る)。多層防御の一枚として更新 zip の完全性検証を入れる。
- **決定**: CI が zip の SHA256 を `k-file-windows.zip.sha256` (sha256sum 形式) として出力し Release に
  添付。アプリは DL 後・展開前に `update_banner._verify_checksum` でサイドカーを取得し
  `updater.sha256_of_file` / `parse_sha256_text` で照合。**不一致 = fail-closed で適用中止 + zip 削除**。
  サイドカー無し (旧 Release) / 取得失敗時は保険なしで続行 (主たる信頼は GitHub 2FA + TLS)。
- **限界 (正直な評価)**: ハッシュを Release に載せる方式は「GitHub アカウント乗っ取りで悪意ある版を
  公開」という本命脅威には**効かない** (攻撃者が zip と hash の両方を書けるため)。通信経路の改ざんは
  既に TLS が守る。よって本 ADR は実質**通信破損への保険**であり、本命の守りは **2FA (済) + 将来の
  コード署名**。SHA256 を「セキュリティ対策」と過信しないこと。
- テスト: `test_updater.py` に pick_sha256_asset / sha256_of_file / parse_sha256_text 系 11 件追加。

### ADR-44: インストーラはユーザー領域 (%LOCALAPPDATA%) に入れ、既存 zip 上書き型 自動更新と両立させる (2026-06-21)
- **背景**: β まで「zip を手動展開」配布だった。正式版に向けインストーラ (スタートメニュー +
  アンインストーラ) を用意したいが、自動アップデート機構 (案②) と整合させる必要がある。
- **決定**: Inno Setup (`installer/k-file.iss`) で `PrivilegesRequired=lowest` +
  `DefaultDirName={localappdata}\Programs\k-file` = **管理者権限なしのユーザー領域インストール**。
  - install_dir がユーザー書き込み可になる → **既存の zip 上書き型 updater がそのまま動く** (Program
    Files だと管理者権限が要り updater を作り直す羽目になる)。= 初回はインストーラ・更新は従来の案②、
    で二重実装を回避。
  - 配布は zip と setup.exe を**両方** Release に置く (zip = 自動更新 + 手動派、setup.exe = 初回導入)。
  - CI は choco で Inno を入れ ISCC でビルド。VERSION は `src/__version__.py` から**行頭アンカー**で抽出
    (docstring の例行を誤検出しないため)。`WizardStyle=classic` でレトロ寄り。
- **割り切り**: コード署名は当面見送り → 未署名インストーラは SmartScreen 警告が強めに出る (ADR-43 と
  併せ、署名が将来の最優先残課題)。差分/silent インストーラ更新 (案③系) も当面保留。
- **実機確認 (2026-06-21)**: setup.exe でインストール → スタートメニュー/デスクトップ起動 → β.11→β.12
  自動更新まで一巡 OK。なお同一 PC に旧 (手動展開) と新 (インストーラ) が併存し得るが、kfile.db は
  `%APPDATA%\k-file` 共有なのでデータは一元。旧フォルダは削除推奨 (自動更新は起動した版だけが上がる)。

### ADR-45: High-DPI はまず PassThrough で OS 倍率追従 (固定px + ビットマップ font 前提の暫定、2026-06-21)
- **背景**: UI はビットマップ font + 固定 px 前提のため、125%/150% の拡大設定でレイアウト崩れ /
  にじみの懸念があった (それまで未検証)。
- **決定**: `main.py` で QApplication 生成前に `setHighDpiScaleFactorRoundingPolicy(PassThrough)` を設定。
  **100% 表示では scale=1.0 で無影響** (= 通常運用に副作用なし)、125/150% では OS 倍率にそのまま追従。
  Win 実機 125/150% で表示崩れが無いことをユーザー確認済 (2026-06-21)。
- **注意**: 抜本対応 (px の論理単位化 / スケーラブル素材化) は将来課題。崩れが報告されたら `Round` 等へ
  切替えて再評価する。

### ADR-46: 表示倍率 (UI scale) は `QT_SCALE_FACTOR` の一律スケール + 再起動反映で実装 (2026-07-01)
- **背景**: ユーザー要望「ウインドウを含め、文字などの表示をパーセンテージで拡大縮小したい」。UI は
  MS ゴシック埋め込みビットマップ 12pt + 全体が固定 px で、バナー/バー offset (ADR-42) 等の緻密な px
  不変条件を持つ。ここで「文字だけ拡大」すると固定 px の行に収まらず崩れる。
- **決定**: Qt の `QT_SCALE_FACTOR` を **QApplication 生成前に** 環境変数で設定し、デバイスピクセル段で
  ウインドウ・文字・全 widget を同じ比率でスケールする。QSS も固定 px も一切書き換えないため、全要素が
  同率で拡大し px 不変条件は比率のまま保たれる (ADR-42 の幾何も崩れない)。既存の High-DPI PassThrough
  (ADR-45) と整合し、OS 倍率に **掛け算** で上乗せされる (例: OS 150% × アプリ 125% = 1.875 倍)。
  - 純ロジックは `src/core/ui_scale.py` (Qt 非依存): `SCALE_STEPS`=75/100/125/150/175/200、
    `clamp_scale`/`parse_scale`/`scale_factor_str`/`step_up`/`step_down`。設定キーは kfile.db の
    `ui_scale_percent`。
  - `main._apply_ui_scale` が起動時に倍率を読んで `os.environ["QT_SCALE_FACTOR"]` へ反映。**100% では
    env を触らない** = 通常運用 (OS 倍率のみ) に一切副作用なし。
  - UI は「表示 → 表示倍率」ラジオ + 拡大 Ctrl++ / 縮小 Ctrl+- / 標準 Ctrl+0。
- **反映は再起動**: `QT_SCALE_FACTOR` は Qt が起動時に一度だけ読むため、runtime での変更不可。倍率変更時は
  確認ダイアログ → 承認で **自動再起動** (`updater.write_relaunch_script` = 更新の relaunch を流用、zip
  展開なしの「旧プロセス消滅待ち→起動」PowerShell)。単一インスタンス IPC (ADR-20) のロック解放を待って
  から起動するので競合しない。closeEvent がタブ/ウインドウサイズを保存 → 新版が復元。配布 .exe のみ自動
  再起動 (開発実行は次回起動で反映)。高倍率で復元サイズが画面をはみ出さないよう availableGeometry で
  クランプ (論理 px 同士なので倍率非依存で比較可)。
- **却下した代替案**: ①QSS を倍率で再生成 + Python 側固定 px を全て scaled() 経由に = 触る箇所が膨大で
  ADR-42 の緻密な px 不変条件を壊すリスク大 (まさにユーザーが最優先で嫌う「業務凍結級の崩れ」)。
  ②font のみ拡大 = 固定 px の行に文字が収まらず崩れる。QT_SCALE_FACTOR 方式はこれらを回避しつつ
  「ウインドウを含め一律」の要件を最小改変で満たす。
- **トレードオフ / 注意**: ビットマップ字形は 100% で最もシャープ。非 100% では埋め込みビットマップの
  無い実 px サイズになりアウトライン (中間/なめらか) 寄りに滲む — ADR-45 で既に許容済の挙動。字形の
  好みは「表示 → 文字の描画」で調整可。125〜200% のレイアウト崩れは Win 実機で要確認 (残タスク §8)。

### ADR-47: ファイル名は「読んだ直後に NFC 正規化」してから日本語リテラルと照合する (2026-07-15)

- **背景**: Mac 実機で「事件タブに文字が出ず、どの事件を開いているか分からない」。フォント/描画の
  問題に見えるが、実際は **タブに入れる文字列そのものが空** だった。`_parse_case` はフォルダ名
  `R08020011文書フォルダ(田中太郎)売買` から目印 `"文書フォルダ"` を探して依頼者名・事件名を切り出す
  が、**macOS の readdir は日本語名を NFD (濁点分解: 「ダ」= 「タ」U+30BF + 濁点 U+3099) で返しうる**。
  ソース中のリテラルは NFC なので `in` 判定が外れ、解析が丸ごと失敗 → `code` = フォルダ名全体 /
  表示名 = 空 → 空タブ。**症状の決定打はパスバー**: NFC 環境なら「R08020011  田中太郎 売買 › …」、
  NFD 失敗時は「R08020011文書フォルダ(田中太郎)売買 › …」とフォルダ名が丸ごと出る。
- **決定**: **ファイルシステム由来の名前は、読んだ直後に `unicodedata.normalize("NFC", …)` してから
  扱う**。Win/Linux は元々 NFC なので正規化は無害 (no-op)。適用箇所: `_parse_case` (タブラベル) /
  `case_repo.resolve_folder` (readdir 名の前方一致。現 case_code は ASCII なので今は無事だが、かなを
  含む命名になると Mac だけ事件を開けなくなる同じ罠)。
- **併せて入れた防御**: `_tab_label(code, name)` = 表示名が空なら code を出す。命名規則に当てはまらない
  フォルダ (スペースも「文書フォルダ」も無い名前) では **Windows でも空タブになり得た** 潜在バグを封鎖。
  「解析が失敗しても文字のないタブにはしない」を構造で保証する。
- **教訓**: パスは OS が正規化差を吸収して開けてしまう (APFS は正規化非依存で lookup できる) ため、
  **「開ける = 文字列として一致する」ではない**。ファイル名を *パースする / 文字列比較する* 箇所だけが
  壊れ、しかも Mac でしか出ない。日本語リテラルとファイル名を突き合わせるコードを増やすときは NFC 必須。

### ADR-48: Mac の自動アップデートは「.app まるごと差し替え」を sh + ditto で行う (2026-07-15)

- **背景**: Mac は自動更新が無く、新版のたびにブラウザ DL → 展開 → 差し替え → `xattr -cr` が必要だった。
  ユーザー要望「せめてアプリ内でボタン一つで更新したい」。Win 版 updater は PowerShell +
  「フォルダ (dist/k-file/) を展開して上書き」前提で、Mac にはそのまま使えない。
- **決定**: **フロー (起動時チェック → バナー → [更新...] → DL → SHA256 照合 → 再起動して適用) は
  Win と完全に同一にし、実行系だけ差し替える**。`write_mac_updater_script` が sh を生成:
  ①`kill -0 $PID` で k-file の終了待ち ②`ditto -x -k` で zip 展開 ③旧 `.app` を `.old` へ退避 →
  新 `.app` を設置 ④`open` で起動 → `.old` 削除。失敗時は **ロールバックして旧版を `open`** (更新に
  失敗してアプリが消えた状態を作らない)。経緯は `~/.config/k-file/updates/updater.log`。
- **`ditto` 必須**: CI も `ditto -c -k` で固めている。Python の zipfile や `unzip` では .app 内の実行
  権限・symlink・拡張属性が落ち、**展開した .app が起動しなくなる**。展開側も `ditto` で対称にする。
- **Gatekeeper は問題にならない**: 検疫マーク `com.apple.quarantine` は「ブラウザ等が DL したファイル」に
  付く印であり、**アプリ自身が DL したファイルには付かない**。よって手動更新で必須だった `xattr -cr` は
  自動更新では不要 (スクリプト側でも念のため `xattr -dr` する)。署名・公証 (年 99 ドル) は自分専用の
  間は引き続き不要。
- **asset の取り違え防止**: Release には `k-file-windows.zip` と `k-file-macos.zip` が両方載る。
  `platform_asset_name()` で OS ごとに選び、`pick_zip_asset` / `pick_sha256_asset` の fallback からは
  **他 OS 向け asset を構造的に除外** (Mac に Windows 版を入れる / 他 OS のハッシュを掴んで
  fail-closed で更新が止まる、を防ぐ)。CI の mac ジョブに `k-file-macos.zip.sha256` を追加。
- **置き場所の制約**: `.app` の親が書き込み不可 (`/Applications` = root 所有) だと差し替えできない。
  k-file を終了する **前に** `os.access(W_OK)` で確認して警告する (終了後に黙って失敗させない)。
  運用は `~/Applications` に置く。
- **副産物**: 表示倍率変更後の自動再起動 (ADR-46) も Mac 版 (`write_mac_relaunch_script`) を用意。
  従来 Mac では PowerShell を起動しようとして失敗し「再起動に失敗」警告が出ていた。なお Mac の
  `open` は env を継承しないため、Win で踏んだ `QT_SCALE_FACTOR` 継承バグ (v1.1.1) は構造的に起きない。
- **Mac 実機なしでの検証方法**: 生成した sh を **Linux 上で実際に実行** して確かめる
  (`tests/test_updater_mac.py`)。`ditto` / `open` / `xattr` の偽コマンドを PATH 先頭に置き、正常系
  (差し替え + 起動 + 後始末) / 展開失敗時のロールバック / 起動中は差し替えを始めない / アポストロフィ
  入りパス を検証。Windows では /bin/sh が無いので skip (CI の test ジョブは ubuntu なので必ず走る)。
...
本アプリの配布形態 (PyInstaller --onedir) は動的リンク相当として LGPL 上問題なし。
